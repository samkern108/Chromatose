...
namespace Chromatose
{
    ...
    public class CameraControl : MonoBehaviour
    {
        ...
        public float SnapDistance = 0.05f; // How far from the new position we should be before snapping to it.
        ...
        // For shaking camera
        private bool _isShaking = false, _isShakingRotation = false;
        private int _shakeCount, _shakeCountMax, _shakeRotationCount;
        private float _shakeIntensity, _shakeSpeed, _baseX, _baseY;
        private float _shakeRotationIntensity, _shakeRotationSpeed;
        private Vector3 _nextShakePosition;
        private float _nextShakeRotation;
        ...
        void Update()
        {
            ...
            /*if (_isShakingRotation)
            {
                // Move toward the previously determined next shake position
                ShakeAxis.localPosition = Vector3.MoveTowards(ShakeAxis.localPosition, _nextShakePosition, Time.deltaTime * _shakeSpeed);

                // Determine if we are there or not
                if (Mathf.Distance(ShakeAxis.localRotation, _nextShakeRotation) < _shakeRotationIntensity / 5f)
                {
                    //Decrement shake counter
                    _shakeRotationCount--;

                    // If we are done shaking, turn this off if we're not longer moving
                    if (_shakeRotationCount <= 0)
                    {
                        _isShakingRotation = false;
                        ShakeAxis.localRotation = new Vector3(0f, 0f, 0f);
                        if (!IsMoving) enabled = false;
                    }
                    // If there is only 1 shake left, return back to base
                    else if (_shakeRotationCount <= 1)
                    {
                        _nextShakePosition = new Vector3(_baseX, _baseY, ShakeAxis.localPosition.z);
                    }
                    // If we are not done or nearing done, determine the next position to travel to
        ...
        {
            float shakeIntensity = _shakeIntensity * Mathf.Pow(((float)_shakeCount / (float)_shakeCountMax), 2);
            _nextShakePosition = new Vector3(Random.Range(-shakeIntensity, shakeIntensity),
        ...
        /// <param name="shakes">Total number of random points the camera will travel to.</param>
        /// <param name="speed">How quickly the camera moves from point to point.</param>
        public void ShakeRotate(float intensity, int shakes, float speed)
        {
            enabled = true;
            _isShakingRotation = true;
            _shakeRotationCount = shakes;
            _shakeRotationIntensity = intensity;
            _shakeRotationSpeed = speed;

            DetermineNextShakeRotation();
        }


        private void DetermineNextShakeRotation()
        {
            _nextShakeRotation = Random.Range(-_shakeRotationIntensity, _shakeRotationIntensity);
        }
    }
}