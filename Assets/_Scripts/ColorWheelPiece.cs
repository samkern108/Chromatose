...
public class ColorWheelPiece : MonoBehaviour {
	...
	void Start () {
		...
		animate.AnimateToPosition (startPosition, finalPosition, 8.0f, Animate.RepeatMode.PingPong);
		animate.AnimateToSize (startScale, finalScale, 8.0f, Animate.RepeatMode.PingPong);
		//animate.AnimateToRotation (transform.rotation, Quaternion.Euler(new Vector3(0, 0, angle + 30)), 6.0f, Animate.RepeatMode.PingPong);
		pieceIndex = pieceCounter;
		pieceCounter++;
	}

	public void OnMouseEnter() {
		animate.AnimateToColor (color, Color.white, .2f, Animate.RepeatMode.Once);
	}

	public void OnMouseExit() {
		animate.AnimateToColor (Color.white, color, .2f, Animate.RepeatMode.Once);
	}
