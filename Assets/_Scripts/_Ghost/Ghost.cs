...
public class Ghost : MonoBehaviour {
	...
	public void EnactRoutine () {
		...
		animate.AnimateToColor (Palette.Invisible,currentColor,.3f, Animate.RepeatMode.Once);
	}
	...
	public void FixedUpdate() {
		if (active && positionIndex >= positions.Length - 3) {
			active = !active;
			animate.AnimateToColor (currentColor, Palette.Invisible, .3f, Animate.RepeatMode.Once);
	...
	private void TelemarkShoot() {
		animate.AnimateToColor (currentColor,Color.red,.3f, Animate.RepeatMode.Once);
	}

	// Lot of duplicate shoot code
	private void Shoot() {
		animate.AnimateToColor (Color.red,currentColor,.3f, Animate.RepeatMode.Once);
