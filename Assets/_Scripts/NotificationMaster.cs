...
public class NotificationMaster : MonoBehaviour {
	...
	public static List<IBeatObserver> beatObservers = new List<IBeatObserver> ();
	...
	public static void SendTickNotification(int level) {
		List<IBeatObserver> toRemove = new List<IBeatObserver> ();
		foreach(IBeatObserver o in beatObservers) {
			if (o.Equals (null))
				toRemove.Add (o);
			else
				o.Tick (level);
		}
		beatObservers = beatObservers.Except (toRemove).ToList ();
	}
}
